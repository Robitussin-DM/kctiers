...
/* end documentation of inline functions */
/* start documentation of KC virtual functions */
...
/* end documentation of KC virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the KC
...
/* start of documentation of KC virtual functions */
...
/* end of documentation of KC virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the KC
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following KC virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs KCping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *KCpedData = &otherData;
  
  // KC both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (KCpedData->first().x() > KCpedData->last().x())
    {
      int size = KCpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*KCpedData)[i], (*KCpedData)[size-1-i]);
    }
    // KC lower bound:
    if (staticData->first().x() < KCpedData->first().x()) // other one must be KCped
      qSwap(staticData, KCpedData);
    int lowBound = findIndexBelowX(KCpedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    KCpedData->remove(0, lowBound);
    // set lowest point of KCped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (KCpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (KCpedData->at(1).x()-KCpedData->at(0).x() != 0)
      slope = (KCpedData->at(1).y()-KCpedData->at(0).y())/(KCpedData->at(1).x()-KCpedData->at(0).x());
    else
      slope = 0;
    (*KCpedData)[0].setY(KCpedData->at(0).y()+slope*(staticData->first().x()-KCpedData->at(0).x()));
    (*KCpedData)[0].setX(staticData->first().x());
    
    // KC upper bound:
    if (staticData->last().x() > KCpedData->last().x()) // other one must be KCped
      qSwap(staticData, KCpedData);
    int highBound = findIndexAboveX(KCpedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    KCpedData->remove(highBound+1, KCpedData->size()-(highBound+1));
    // set highest point of KCped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (KCpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = KCpedData->size()-1; // last index
    if (KCpedData->at(li).x()-KCpedData->at(li-1).x() != 0)
      slope = (KCpedData->at(li).y()-KCpedData->at(li-1).y())/(KCpedData->at(li).x()-KCpedData->at(li-1).x());
    else
      slope = 0;
    (*KCpedData)[li].setY(KCpedData->at(li-1).y()+slope*(staticData->last().x()-KCpedData->at(li-1).x()));
    (*KCpedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (KCpedData->first().y() < KCpedData->last().y())
    {
      int size = KCpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*KCpedData)[i], (*KCpedData)[size-1-i]);
    ...
    // KC lower bound:
    if (staticData->first().y() > KCpedData->first().y()) // other one must be KCped
      qSwap(staticData, KCpedData);
    int lowBound = findIndexAboveY(KCpedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    KCpedData->remove(0, lowBound);
    // set lowest point of KCped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (KCpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (KCpedData->at(1).y()-KCpedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (KCpedData->at(1).x()-KCpedData->at(0).x())/(KCpedData->at(1).y()-KCpedData->at(0).y());
    else
      slope = 0;
    (*KCpedData)[0].setX(KCpedData->at(0).x()+slope*(staticData->first().y()-KCpedData->at(0).y()));
    (*KCpedData)[0].setY(staticData->first().y());
    
    // KC upper bound:
    if (staticData->last().y() < KCpedData->last().y()) // other one must be KCped
      qSwap(staticData, KCpedData);
    int highBound = findIndexBelowY(KCpedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    KCpedData->remove(highBound+1, KCpedData->size()-(highBound+1));
    // set highest point of KCped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (KCpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = KCpedData->size()-1; // last index
    if (KCpedData->at(li).y()-KCpedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (KCpedData->at(li).x()-KCpedData->at(li-1).x())/(KCpedData->at(li).y()-KCpedData->at(li-1).y());
    else
      slope = 0;
    (*KCpedData)[li].setX(KCpedData->at(li-1).x()+slope*(staticData->last().y()-KCpedData->at(li-1).y()));
    (*KCpedData)[li].setY(staticData->last().y());
  }
