...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeKrakenAmount"))
        settings.setValue("nAnonymizeKrakenAmount", 1000);
    nAnonymizeKrakenAmount = settings.value("nAnonymizeKrakenAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeKrakenAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeKrakenAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeKrakenAmount:
            return QVariant(nAnonymizeKrakenAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeKrakenAmount:
            nAnonymizeKrakenAmount = value.toInt();
            settings.setValue("nAnonymizeKrakenAmount", nAnonymizeKrakenAmount);
            emit AnonymizeKrakenAmountChanged(nAnonymizeKrakenAmount);
            break;
        default:
            break;
        }
